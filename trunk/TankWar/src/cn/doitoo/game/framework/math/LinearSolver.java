...
public class LinearSolver {
	...
	public static Point vectorIntsecPoint(float x1,float y1,float x2,float y2,float x3,float y3,float x4,float y4){
	    ...
	    if(distance(x1,y1,p.x,p.y)<distance(x2,y2,p.x,p.y)||distance(x3,y3,p.x,p.y)<distance(x4,y4,p.x,p.y))return null;
		return p;
	}
	...
	public static float distance(float x1,float y1,float x2,float y2){
		return FloatMath.sqrt((float) (Math.pow(x1-x2, 2)+Math.pow(y1, y2)));
	}
