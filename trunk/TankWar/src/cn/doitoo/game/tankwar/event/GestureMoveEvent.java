...
public class GestureMoveEvent extends ITouchEventHandler {
    ...
    public void onTouchMove(MotionEvent event) {
        ...
        float px = Math.min(Math.max(map.getX() + deltaX, maxWidth), 48);
        float py = Math.min(Math.max(map.getY() + deltaY, maxHeight), 48);
    ...
    public void onTouchUp(MotionEvent event) {
        ...
        if (Math.abs(x - preX) < rect && Math.abs(y - preY) < rect) {
            ...
            if (player != null) {
                ...
                if (playerCurrentRect.contains((int) x, (int) y)) {
                    return;
                }
    ...
    public void computeShortestPath(Point startNodePoint, Point endNodePoint) {
        int startNode = Util.worldPoint2Node(map, startNodePoint, gameMap01Vector);

        int endNode = Util.worldPoint2Node(map, endNodePoint, gameMap01Vector);
